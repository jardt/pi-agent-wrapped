...
Wrapped Pi stores sessions under the active `PI_CODING_AGENT_SESSION_DIR`, organized by project:
...
```json
{
  "dir": "$PI_CODING_AGENT_SESSION_DIR/<project>/subagent-artifacts",
  "files": [
    {
...
3. **Persistent artifacts** — `details.artifacts.files[]` under `$PI_CODING_AGENT_SESSION_DIR/<project>/subagent-artifacts/` (persistent)
